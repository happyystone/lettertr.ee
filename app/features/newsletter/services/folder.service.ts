...
export class FolderService {
  ...
  static async getUserFolders(request: Request): Promise<UserFolder[]> {
    return executeWithAuth(request, async (db, user) => {
      try {
        const folders = await db((tx) =>
          tx
            .select()
            .from(userFolders)
            .where(eq(userFolders.userId, user.id))
  ...
  static async createFolder(
    ...
    return executeWithAuth(request, async (db, user) => {
      try {
        ...
        const maxSortOrder = await db((tx) =>
          tx
      ...
      try {
        const [folder] = await db((tx) =>
          tx
            .update(userFolders)
            .set(data)
            .where(and(eq(userFolders.id, folderId), eq(userFolders.userId, user.id)))
            .returning(),
        );
      ...
      try {
        ...
        const newsletters = await db((tx) =>
          tx
            ...
            .where(
              and(eq(newsletterFolders.folderId, folderId), eq(userNewsletters.userId, user.id)),
            ),
        );
  ...
  static async addNewsletterToFolder(
    ...
    return executeWithAuth(request, async (db, user) => {
      try {
        ...
        let userNewsletter = await db((tx) =>
          tx
            .select()
